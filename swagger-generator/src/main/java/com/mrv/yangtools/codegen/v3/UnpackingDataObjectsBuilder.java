...
public class UnpackingDataObjectsBuilder extends AbstractDataObjectBuilder {
     ...
     */
    public UnpackingDataObjectsBuilder(SchemaContext ctx, OpenAPI swagger, TypeConverter converter) {
    ...
    /**
     * Build OpenAPI schema for given Yang data node
     * @param node for which we want to build model
     * @param <T> YANG node type
     * @return Swagger model
     */
    public <T extends SchemaNode & DataNodeContainer> Schema<String> build(T node) {
    	Schema<String> schema = new Schema<String>();
    	schema.description(desc(node));
    	//TODO DM port to OpenAPI form previous
    	schema.properties(structure(node));
    ...
    protected <T extends DataSchemaNode & DataNodeContainer> Schema<?> refOrStructure(T node) {
        ...
        if(useReference) {
            ...
            schema = new Schema<>().$ref(definitionId);
        } else {
            log.debug("submodel for {}", getName(node));
            
            schema = new ObjectSchema().properties(structure(node, x -> true, x -> true));
        }
        return schema;
    }
